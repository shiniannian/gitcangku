...
//        循环读取游标内容，存储到对象当中
    ...
    * */
    ...
    /*
    * 获取记账表当中某一天的所有支出或者收入情况（用于MainActivity中mDatas的数据源）
    * */
    public static List<AccountBean>getAccountListOneDayFromAccounttb(int year,int month,int day){
        List<AccountBean>list = new ArrayList<>();
        String sql = "select * from accounttb where year=? and month=? and day=? order by id desc"; //查询语句
        Cursor cursor = db.rawQuery(sql, new String[]{year + "", month + "", day + ""});        //得到Cursor（光标，游标）对象
        //遍历符合要求的每一行数据
        while (cursor.moveToNext()) {
            int id = cursor.getInt(cursor.getColumnIndex("id"));   //得到它的id
            String typename = cursor.getString(cursor.getColumnIndex("typename"));
            ...
            AccountBean accountBean = new AccountBean(id, typename, sImageId, beizhu, money, time, year, month, day, kind); //创建AccountBean对象（自定义的）
    ...
    public static float getSumMoneyOneDay(int year,int month,int day,int kind){
        float total = 0.0f;     //总金额
        String sql = "select sum(money) from accounttb where year=? and month=? and day=? and kind=?"; //查询
